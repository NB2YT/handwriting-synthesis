...
def raw_rnn(cell, loop_fn, parallel_iterations=None, swap_memory=False, scope=None):
    ...
    with vs.variable_scope(scope or "rnn") as varscope:
        ...
        for input_shape_i in input_shape:
            ...
            if static_batch_size is not None and input_shape_i[0] is not None:
                if not tf.compat.v1.Dimension(static_batch_size).is_compatible_with(input_shape_i[0]):
                    raise ValueError(f"Batch sizes are incompatible: {static_batch_size} vs {input_shape_i[0]}")
